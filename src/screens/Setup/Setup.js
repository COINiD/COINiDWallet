...
import {
  Alert, View, Linking, Image, TouchableOpacity, ScrollView, Platform,
} from 'react-native';
...
import {
  SelectAddressType, SetupWallet, InputPublicKey,
} from '../../dialogs';
...
class Setup extends PureComponent {
  ...
  _buildWallet = (data) => {
    const createAccount = (pubKeyArray) => {
      ...
      const createWallet = () => setBuildStatus('Setting up your account...')
        .then(() => {
  ...
  _coldView = ({
    ...
    const renderButton = () => {
      let buttonText = 'Start setup of cold wallet';
      ...
      <View style={styles.container}>
        ...
        >
          <View style={styles.coldLottieWrapper}>
            <LottieView
              style={{}}
              ref={(c) => { this.lottieAnim = c; }}
              source={lottieFiles.setupcold}
              loop={false}
            />
          </View>
          <Text h1 center margin>Welcome</Text>
          <Text h2 center margin>Setup your Cold Wallet</Text>
          ...
          <Text p center margin>
            Your cold wallet private keys are never exposed to an online device.
            Offline storage is the safest way to store your funds.
          </Text>
        </ScrollView>
    ...
    const renderTransportContent = ((renderArgs) => {
      ...
      return (
        <React.Fragment>
          ...
          <SelectAddressType
            ref={(c) => { this.addressTypeModal = c; }}
            onSelectAddressType={this._onReturnFromSelectAddressType}
          />
          <SetupWallet
            ref={(c) => { this.setupWalletModal = c; }}
            onContinue={this._selectAddressType}
            onContinuePublic={this._enterPublicKey}
            isBLESupported={isBLESupported}
          />
          <InputPublicKey
            ref={(c) => { this.inputPublicKeyModal = c; }}
            onContinue={(data) => { const pubKeyData = data.split('://')[1]; this._handleReturnData(pubKeyData); }}
          />
        </React.Fragment>
      );
    });
