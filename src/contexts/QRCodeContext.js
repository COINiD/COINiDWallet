...
export class QRCodeProvider extends PureComponent {
  static propTypes = {
  ...
  constructor() {
    ...
    this.state = {
      value: {
        getDataURL: this._getDataURL,
      },
    };
  }

  _getDataURL = params => new Promise((resolve) => {
    const key = `${parseInt(Math.random() * 100000, 10)}`;

    try {
      this.webView.injectJavaScript(getInjection(key, params));

      this.callbacks[key] = (uri) => {
        resolve(uri);
      };
    } catch (err) {
      // err
  ...
  _receiveJSMessage = ({ nativeEvent }) => {
    const { data } = nativeEvent;
    const { uri, key } = JSON.parse(data);
      ...
      <QRCodeContext.Provider value={value}>
        ...
        <View style={styles.webView}>
          <WebView
            ...
            onMessage={this._receiveJSMessage}
            injectedJavaScript={INJECTION_LIB}
            useWebKit
            javaScriptEnabled
...
export class QRCode extends Component {
  static contextType = QRCodeContext;
  ...
  static propTypes = {
    ...
    renderingSize: PropTypes.number,
  };
  ...
  componentDidMount() {
    this._updateUri(this.props);
  }

  shouldComponentUpdate(nextProps, nextState) {
    const { uri } = this.state;
    const { uri: nextUri } = nextState;

    if (this.props !== nextProps) {
      this._updateUri(nextProps);
    }

    return uri !== nextUri;
  }

  _updateUri = async (props) => {
    const { getDataURL } = this.context;
    const uri = await getDataURL(props);
    this.setState({ uri });
  };
